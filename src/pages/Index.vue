          ...
          <div id="doc-oc-demo3" class="am-offcanvas">
            <div class="am-offcanvas-bar am-offcanvas-bar-flip">
              <div class="am-offcanvas-content">
                <p>
                  <a href="http://music.163.com/#/song?id=385554" target="_blank"></a>
                </p>
              </div>
            </div>
          </div>
        ...
        <div class="windows_body" ref="windowsBody">
          <div class="office_text message_view" style="height: 100%;">
            <ul ref="officeText" class="content" id="chatbox">
              <li
                :class="[message.sender_uid==user.userId?'me':'other']"
              ...
              <div v-else>
                ...
                >
                <span class="content_box" v-if="message.msgType==2001" @contextmenu="showTools($event,message)">{{message.content}}</span>
                <span class="img_box content_box" v-else-if="message.msgType==2002">
                  <img :src="message.content" alt>
					...
					<ul v-show="showMenu" ref="contextmenu" class="menu-ul">
						<li class="menu-li" v-show="menuItem.show" v-for="(menuItem,key) in contextmenu" :key="key" @click="clickMenuEvent(menuItem)">
							{{menuItem.eventName}}
						</li>
					</ul>
        </div>
  ...
  methods: {
    ...
    async buildTalkView(uid) {
      let _this = this;
			...
			this.activeMessageList = this.messageJson[uid]?this.messageJson[uid].msgs:[];
      setTimeout(function() {
    ...
    logOutIm() {
      ...
      strophe.logOutIm("主动登出");
		...
		showTools(e,msg){
			if(this.activeMessageViewType == "groupchat"){
				if(this.groupMembers[0].userId==this.user.userId||msg.sender_uid==this.user.userId) {
